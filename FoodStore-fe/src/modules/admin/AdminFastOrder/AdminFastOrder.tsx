...
import { columns } from '../config/fastOrder';
import { ORDER_FILTER_DEFAULT } from '../config/order';
import AdminFastOrderCart from './AdminFastOrderCart';
import AdminFastOrderUpdate from './AdminFastOrderUpdate';
import './index.scss';


const AdminFastOrder = () => {
  const { Search } = Input;
  ...
  return (
    <>
      ...
      />
      {modal.edit && <AdminFastOrderUpdate data={order} handleCancel={turnOffModal} handleOk={updateHandler}></AdminFastOrderUpdate>}
      {modal.cart && order.code && <AdminFastOrderCart data={{ ...order, type: OrderType.FAST_ORDER }} handleOk={turnOffModal} handleCancel={turnOffModal}></AdminFastOrderCart>}
    </>
  )
}

export default AdminFastOrder