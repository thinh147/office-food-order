...
const mapper = CHANNEL_FORM.reduce(
  (acc, cur) => ({ ...acc, [cur.value]: cur.label }),
  {} as { [key in Channel]: string }
);
...
const ProductItem = ({
  ...
  return (
    <Col
      span={6}
      className="product_Item_information pointer"
      onClick={handleClick}
    >
      <div className="image">
      ...
      <div>
        <div className="price">
          {CurrencyWithCommas((price * (100 - (percentDiscount || 0))) / 100)}{" "}
          <sup>đ</sup>
        </div>
        <div className="price text-black text-cross">
          {percentDiscount > 0 ? (
            <span>
              {CurrencyWithCommas(price)} <sup>đ</sup>
            </span>
          ) : (
            <></>
          )}
